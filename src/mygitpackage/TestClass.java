package mygitpackage;

public class TestClass {

	public static void main(String[] args) {
		System.out.println("Version 1");
		System.out.println("Git' den Merhaba");
		System.out.println("Github' tan Merhaba");// Bu benim Github.com daki yeni kodum
		System.out.println("Uzak sunucudaki kodum");// Bu benim Github.com daki yeni kodum
		System.out.println("10. SATIR bilgisayarim");// Bu benim Github.com daki yeni kodum
		
		
		System.out.println("13. satir bilgisayarim");// Bu benim Github.com daki yeni kodum
		
		System.out.println("NESE BRANCHINDE YAPTIGIM KODLAR");
	}

}
